...
export default class extends Controller {
  static targets = ["row", "tab", "existingSelect", "newName", "projectNameField"]
  ...
  selectExisting(event) {
    const row = event.target.closest(".lookout-manager__row")
    if (!row) return
    const radio = row.querySelector("input[name^='lookout-dest'][value='existing']")
    if (radio && !radio.disabled) { radio.checked = true; this._syncRow(row) }
  }
  ...
  selectNew(event) {
    ...
    if (radio) { radio.checked = true; this._syncRow(row) }
  }
  ...
  newNameChanged(event) {
    ...
    const row = event.target.closest(".lookout-manager__row")
    if (!row) return
    const field = row.querySelector("[data-lookout-manager-target='projectNameField']")
  ...
  _syncDisplay() {
    ...
    this.tabTargets.forEach(tab => {
      tab.classList.toggle("lookout-manager__tab--active", tab.dataset.rowId === this._activeId)
    })
  }

  _syncRow(row) {
    const radio = row.querySelector("input[name^='lookout-dest']:checked")
    const existingSelect = row.querySelector("[data-lookout-manager-target='existingSelect']")
    const newInput = row.querySelector("[data-lookout-manager-target='newName']")
    const field = row.querySelector("[data-lookout-manager-target='projectNameField']")
